...
from tabito_itemgen.exam_models import ExamHumanQA, ExamQAChecks, Q1Section, Q2OrderingTask, Q2Section, Q3Section, Q5Section
...
from tabito_itemgen.exam_review_models import SECTION_SPECIFIC_QA, SectionHumanQA, SectionQAChecks
...
from tabito_itemgen.ui_app import APP_CSS, OPTION_MARKS, _render_booklet_preview, _render_teacher_view
...
def _discover_exams(root: Path) -> dict[str, Path]:
    ...
    for state in ("draft", "approved"):
        ...
        for path in sorted(directory.glob("*/exam.json"), reverse=True):
            ...
            except Exception:  # UI discovery must not hide every other healthy exam because one draft is broken.
                continue
...
def _section_status(root: Path, manifest, exam_path: Path, ref) -> tuple[str, str]:
    ...
    if not validation.passed:
        return "结构修正", "bad"
    if "draft" not in exam_path.parts:
        return "APPROVED", "ok"
    try:
...
def _html_options(options: list[str]) -> None:
    ...
    for index, option in enumerate(options, start=1):
        ...
        rows.append(
            f'<div class="option-row"><div class="option-mark">{mark}</div><div>{html.escape(option)}</div></div>'
        )
...
def _preview_simple_section(section, teacher: bool = False) -> None:
    st.markdown(
        f'<div class="exam-header exam-title"><b>第{section.section[1]}問</b>　{html.escape(section.title_ja)}（配点 {section.score}）</div>',
    ...
    if isinstance(section, Q1Section):
        ...
        for task in sorted(section.tasks, key=lambda value: value.answer_slot.answer_number):
            ...
            if hasattr(task, "headword"):
                st.markdown(
                    f'<div class="exam-prompt">{html.escape(task.prompt_ja)} {_answer_badge(task.answer_slot.answer_number)}</div>',
                    unsafe_allow_html=True,
                )
            ...
            if teacher:
                st.caption(f"正答 [{task.answer_slot.answer_number}] {task.answer_slot.correct_option} · {task.rationale_ja}")
        return
    if isinstance(section, Q2Section):
        for task in sorted(section.tasks, key=lambda value: value.order):
            ...
            if isinstance(task, Q2OrderingTask):
                ...
                st.markdown(f'<div class="exam-prompt">{html.escape(task.prompt_ja)} {badges}</div>', unsafe_allow_html=True)
                st.markdown(task.source_ja)
                st.markdown(f"{task.sentence_frame_zh}")
                st.markdown("　".join(f"{OPTION_MARKS[token.token_id - 1]} {token.text_zh}" for token in task.token_pool))
                if teacher:
                    st.caption("正答 " + " / ".join(f"[{slot.answer_number}] {slot.correct_option}" for slot in task.answer_slots))
            else:
                st.markdown(f'<div class="exam-prompt">{html.escape(task.prompt_ja)} {_answer_badge(task.answer_slot.answer_number)}</div>', unsafe_allow_html=True)
    ...
    if isinstance(section, Q5Section):
        for paragraph in section.paragraphs:
            st.markdown(f'<div class="source-text">{html.escape(paragraph.text_zh)}</div>', unsafe_allow_html=True)
        st.markdown("---")
        for task in sorted(section.tasks, key=lambda value: value.question_no):
            ...
            _html_options(task.options)
            if teacher:
                st.caption("正答 " + " / ".join(f"[{slot.answer_number}] {slot.correct_option}" for slot in task.answer_slots) + f" · {task.rationale_ja}")
...
def _section_human_qa_form(root: Path, manifest, ref, section) -> None:
    ...
    for field in SectionQAChecks.model_fields:
        common_values[field] = st.checkbox(field.replace("_", " "), key=f"qa-{ref.section}-{field}")
    st.markdown("**Section-specific**")
    specific = {}
    for field in SECTION_SPECIFIC_QA[ref.section]:
        specific[field] = st.checkbox(field.replace("_", " "), key=f"qas-{ref.section}-{field}")
    reviewer = st.text_input("Reviewer", value="TABITO 教研", key=f"reviewer-{ref.section}")
    disposition = st.selectbox("Disposition", ["revise", "approve", "reject"], key=f"disp-{ref.section}")
    defects = st.text_input("Defects（逗号分隔）", key=f"defects-{ref.section}")
    if st.button("保存 Human QA", key=f"save-qa-{ref.section}", width="stretch"):
        qa = SectionHumanQA(
            section=ref.section,
...
def _create_exam_panel(root: Path) -> None:
    ...
    with left:
        family = st.radio("2026 blueprint", ["main_2026", "makeup_2026"], format_func=lambda value: "2026 本試験型" if value == "main_2026" else "2026 追試験型", horizontal=True)
...
def main() -> None:
    ...
    root = _root()
    st.markdown('<div class="tabito-kicker">TABITO EDUCATION · EXAM PRODUCTION</div>', unsafe_allow_html=True)
    st.markdown('<div class="tabito-title">共通テスト中国語 模試制作 Workbench</div>', unsafe_allow_html=True)
    st.markdown('<div class="tabito-sub">2026 本試験＋追試験を一次蓝本に、Q1–Q5を別々に作り、1冊の200点模試としてreleaseする。</div>', unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        + _status_badge("80分")
        + _status_badge("50解答欄")
        + _status_badge(manifest.workflow.state.upper(), "ok" if manifest.workflow.state == "approved" else "")
        + "</div>",
        unsafe_allow_html=True,
    )
    ...
    with section_tab:
        section_name = st.selectbox("大题", [ref.section for ref in manifest.sections], format_func=lambda value: f"{value} · {SECTION_LABELS[value]}")
        ...
        if path is None or not path.exists():
            ...
            st.download_button("下载生成 Prompt", request_path.read_text(encoding="utf-8"), file_name=request_path.name, width="stretch")
            response = st.text_area("把 ChatGPT 返回的 JSON 粘贴到这里", height=320, key=f"response-{section_name}")
            if st.button("导入并校验", type="primary", key=f"import-{section_name}", width="stretch"):
                try:
            ...
            student_view, teacher_view, review_view = st.tabs(["学生预览", "教师标注", "Review / QA"])
            with student_view:
            ...
            with review_view:
                ...
                st.success("Deterministic QA PASS") if result.passed else st.error("Deterministic QA FAIL")
                for error in result.errors:
                    st.markdown(f"- {error}")
                review_request = create_section_review_request(root, manifest.exam_id, section_name)
                st.download_button("下载 Blind Review Prompt", review_request.read_text(encoding="utf-8"), file_name=review_request.name, width="stretch")
                review_json = st.text_area("粘贴 reviewer JSON", height=220, key=f"review-json-{section_name}")
                if st.button("导入 Blind Review", key=f"review-import-{section_name}", width="stretch"):
                    try:
                        import_section_review(root, manifest.exam_id, section_name, review_json)
                        st.success("Blind Review 已绑定当前版本")
                        st.rerun()
                    except Exception as exc:
                        st.error(str(exc))
                try:
                    readiness = section_release_readiness(root, manifest.exam_id, section_name)
                    for gate in readiness.gates:
                        st.markdown(_status_badge(("✓ " if gate.passed else "○ ") + gate.name, "ok" if gate.passed else "warn"), unsafe_allow_html=True)
                    _section_human_qa_form(root, manifest, ref, section)
                    if not readiness.ready:
                        revision = create_section_revision_request if (root / "workspace" / "exams" / manifest.exam_id / "reviews" / f"{section_name.lower()}.review.json").exists() else None
                        if revision and st.button("生成 Revision Prompt", key=f"revise-{section_name}"):
                            out = revision(root, manifest.exam_id, section_name)
                            st.download_button("下载 Revision Prompt", out.read_text(encoding="utf-8"), file_name=out.name)
                except Exception as exc:
                    st.warning(str(exc))

    with release_tab:
        st.markdown("### Final Exam QA")
        st.caption("所有 section ready 后，再从整卷角度检查80分钟负荷、跨大题重复、版面与统一性。")
        qa_values = {name: st.checkbox(name.replace("_", " "), key=f"examqa-{name}") for name in ExamQAChecks.model_fields}
        reviewer = st.text_input("Exam QA reviewer", value="TABITO 教研")
        disposition = st.selectbox("Exam disposition", ["revise", "approve", "reject"])
        ...
        try:
            ...
            if readiness.ready and st.button("Approve → 正式模試库", type="primary", width="stretch"):
        ...
        if all(ref.path for ref in manifest.sections):
                ...
                try:
                    ...
                    for key, output in outputs.items():
                        if output and output.exists():
                            ...
                            st.download_button(f"下载 {key}", output.read_bytes(), file_name=output.name, mime=mime)
                except Exception as exc:
                    st.error(str(exc))
