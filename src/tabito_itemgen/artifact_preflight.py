...
class ArtifactManifest(BaseModel):
    ...
    def require_current_renderer(self) -> ArtifactManifest:
        current = renderer_revision()
        if current == "unknown":
            raise ValueError("current renderer revision cannot be resolved")
        if self.renderer_revision != current:
            raise ValueError(
                "artifact was generated by a stale renderer: "
                f"{self.renderer_revision!r} != current {current!r}"
            )
        return self
