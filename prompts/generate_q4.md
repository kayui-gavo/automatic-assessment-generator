...
以前の失敗は、換皮を避けるために2026のsurface grammarまで捨て、一般的な「多資料読解」にしてしまったことだった。今回は逆に、**題材・文章・数値・固有名詞・正誤関係は原创にしつつ、設問骨格は選択した2026 familyに十分近づける。**
...
`Item Spec` に指定された family を厳守する。`auto` の場合はどちらか一方を選び、混合型にはしない。
...
- 調査・資料読解の4枠
- 表、グラフ、説明文＋図などを使う
- `main_2026`：原則 2枠 + 1枠 + 1枠
- `makeup_2026`：原則 2枠 + 2枠
- 四つの独立単選問題に分解しない
...
### `main_2026`
- 問1 29・30：checklist / 条件資料を読んで二つ選ぶ
- 問2 31〜33：profile / candidate / case と希望・条件を照合して2枠、その照合過程から「追加すべき情報項目」等を1枠
- 問3 34〜36：flowchart / rule tree / process を読み、一般原則1枠 + 二つの具体caseへの適用2枠

### `makeup_2026`
- 問1 29・30：SNS / timetable / 日付 / 曜日 / 条件等を組み合わせ、実際の計画・日時・行動を二つ選ぶ
- 問2 31〜34：map / memo / system diagram等の複合資料で2枠、その後 flyer / instructions / safety notice 等の別資料で2枠
- 問3 35・36：発表・活動後のreflection / summary / opinion textから二つ選ぶ
...
- 中国語は自然な現代簡体字。
- 日本語設問は大学入試センターの冊子に置いて違和感のない簡潔な試験文体。
- 中国語理解を問う選択肢は、必要がない限り中国語文として完結させる。
- 語注は必要最小限。
...
3. 2026本試/追試の本文やcaseの換皮ではなく、内容は原创か。
4. 逆に原创性を意識しすぎて一般的なworksheetになっていないか。
5. 日本語設問・中国語資料・選択肢の見た目が共通テスト冊子の情報密度に近いか。
6. 正答が一意か。
