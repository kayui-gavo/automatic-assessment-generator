# TABITO 共通テスト中国語 Q4 生成タスク v0.3.2
...
8. **資料形式は情報関係から決める。** 単なる「名称→分類」「項目→説明」の一覧で十分な情報を、見栄えのためだけに map / annotated_diagram / flowchart にしない。位置関係・経路・因果・階層・手順など、視覚表現でなければ読み取りにくい関係があるときだけ視覚資料を使う。
9. full では A・B それぞれに少なくとも1つ、単純な一資料の語句一致では解けない統合的 task を置く。
10. multi_select は2026両試験で重要だが、形式だけを真似しない。複数の記述を個別に検証させる必要があるときに使う。
11. 数値資料は計算力ではなく中国語での情報処理を測る。比較・割合・増減・条件判断を中心にし、重い計算を要求しない。
12. 中国語は自然な現代簡体字。専門語・低頻度語は必要に応じて glosses を付ける。HSK練習問題のような文法ドリル感を避ける。
13. 日本語の設問文は大学入試として簡潔・自然にする。説明口調・AI口調は禁止。
14. 選択肢は、意図がない限り一文の中で日本語文法と中国語文法を不自然に混在させない。中国語理解を問う選択肢なら中国語文として成立させ、日本語は設問指示や必要な補助説明に使う。
15. 正答または正答集合は一意。誤答肢は、部分一致・条件取り違え・範囲の読み違い・因果逆転・過剰推論など、現実的な誤読経路を持たせる。
16. multi_slot_choice で共通選択肢を使う場合、**選択肢を繰り返し使用できるかどうかを設問文に明記**し、`option_reuse` と題面を一致させる。
17. 統計・制度・組織名を使う場合、架空の数値を実在機関の公式データのように見せない。内部原创の synthetic data ならその前提で設計する。
18. evidence は資料IDだけでなく locator に、該当発話・表セル・投稿・図の関係・条件等を人間が確認できる粒度で記す。
19. **下記 Generation Profile は2026二試験から抽象化済みである。公式問題の逐問配列を推測・復元し、それに沿って新題を並べない。**
20. JSON以外を出力しない。Markdown fence、前置き、後書きは禁止。
...
- **図・地図・フローが、同じ情報を表や短いリストにしても何も失わない「飾り資料」になっていないか**
- A/Bそれぞれに統合的な情報処理があるか
- 後半が「同じ読解の続き」だけになっていないか
- multi_slot_choice の選択肢再利用ルールが題面と metadata で一致しているか
- 正答集合に曖昧さがないか
