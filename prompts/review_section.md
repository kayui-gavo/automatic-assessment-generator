...
7. `cross_source` / 複数資料統合を名乗る設問では**source ablation**を行う。引用・提示されている資料を一つずつ隠したと仮定し、残りだけで正答が一意に決まらないか確認する。一資料を隠しても正答が一意のままなら、実質的な cross-source ではない。`shortcut` または `information_dependency` の issue として記録し、原則 `revise` にする。
...
- **各 cross-source task について資料を一つずつ隠す消融を必ず行う。単一資料だけで選択肢を一つまで絞れるなら revise。**
